...
Obviously the most important dependency we need is `react` but along with that we also need `react-dom` (which was introduced with React version 0.14).  We're also going to be using `react-router` so we'll include that as well.
...
> #### What is JSX?
> JSX is React's version of a hybrid JavaScript/HTML combo.  You can use most general JavaScript within your HTML markup.  This includes merging variables, conditional statements, loops (for, etc), etc.  One thing you can't do is use normal `if` statements.  For this you need to use terciary statements (`{myvar ? 'show this if true' : 'else this if false}`).
...
With this basic snippet of code, we import the React and ReactDOM modules and execute `ReactDOM.render` passing in first a reference to our App component and second the DOM element we want to inject our rendered HTML into (in this case, a div with an id of `app`).  If you recall from the earlier chapter we created the `server/public/mrw/index.html` file.
...
Before we can give our new component a spin, we need to adjust our `webpack.config.js` file to point to `main.js` file instead of the original `sample.js` file from chapter 5:
...
Let's first our first child component, which we will use within the App component.  Start by creating and editing `client/components/layout/Header.js` and inserting the following code:
...
A few new things are in this `Header` component.  First we included a reference to React.PropTypes because React typically expects all props that are expected to be predefined.  We define them via a `static` property on our component class.  We didn't include this in the `App` component even though it is technically required, and you might have noticed a warning in the webpack output when you ran a build.
...
Let's now use this new Header component in our `App` component.  Edit `client/js/components/App.js` and change it to look like the following:
...
In addition to events, you're inevitiably going to want to manage some kind of state for your component.  This could be something as simple as a toggle state for a checkbox or show/hide state for an accordion etc.  To manage state simply use a component's `this.state` object and define your own properties.

Let's ajust the `clickLogger` within `Header` to count the number of times a message was logged and display that counter in our actual `H1` tag:
